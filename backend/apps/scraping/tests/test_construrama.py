...
def test_command_corrida_real_construrama_crea_observations(cr_setup, monkeypatch):
    ...
    assert run.status == ScrapeRun.Status.OK
    assert run.items_found == 7
    # El comando sigue auditando su origen como 'command' (default). F035: ahora
    # además pasa `--category` como `search_term` (antes null, deuda de F033) para
    # que el filtro por término de la búsqueda también aplique a sus datos.
    assert run.triggered_by == ScrapeRun.TriggeredBy.COMMAND
    assert run.search_term == "varilla"
